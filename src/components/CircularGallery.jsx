...
class Title {
  ...
  createMesh() {
    if (!this.text) return; // Segurança extra se não houver texto
...
class Media {
  ...
  createShader() {
    ...
    img.src = this.image;
    img.onload = () => {
      if (!this.gl) return; // 🔥 Evita carregar se o componente foi desmontado
      texture.image = img;
  ...
  update(scroll, direction) {
    ...
    this.program.uniforms.uTime.value += 0.02; // 🔥 Reduzido ritmo para economizar CPU mobile
...
class App {
  ...
  createRenderer() {
    this.renderer = new Renderer({ alpha: true, antialias: true, dpr: Math.min(window.devicePixelRatio || 1, 1.5) }); // 🔥 DPR travado em max 1.5 no mobile economiza VRAM
  ...
  createGeometry() {
    this.planeGeometry = new Plane(this.gl, { heightSegments: 20, widthSegments: 40 }); // 🔥 Reduzida a densidade de polígonos (perfeito para mobile)
  }
  ...
  addEventListeners() {
    ...
    // 🔥 Captura os toques apenas no container específico da galeria, salvando a CPU do mobile
  ...
  destroy() {
    ...
    // 🔥 Força a limpeza de memória das texturas WebGL da GPU antes de excluir o canvas
    if (this.medias) {
      this.medias.forEach(m => {
        if (m.program) m.program.remove();
        if (m.plane) m.plane.remove();
      });
    }
...
export default function CircularGallery({
  ...
  return <div className="circular-gallery" ref={containerRef} style={{ width: '100%', height: '100%', overflow: 'hidden' }} />;
}